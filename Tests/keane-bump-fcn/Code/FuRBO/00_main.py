...
cwd_current = os.path.join(cwd_base, 'keane')
    
# Check if directory already exists
if any([True if dir_ == 'keane' else False for dir_ in os.listdir(cwd_base)]):
    # Check if problem was already solved
    if any([True if dir_ == 'complete' else False for dir_ in os.listdir(os.path.join(cwd_base, 'keane'))]):
...
for i, seed in enumerate(seeds):
    ...
    # if 'keane' + '_it_' + str(i) + '.torch' in os.listdir(os.path.join(cwd_base, 'keane')):
    #     continue
    ...
    # Define objective
    obj = keane(dimension = 30,
                lower_bound = torch.zeros(30),
                upper_bound = 10 * torch.ones(30))
    
    # Define constraint
    cons = constraints.keane_cons(dimension = 30,
                                  lower_bound = torch.zeros(30),
                                  upper_bound = 10 * torch.ones(30))
    ...
    filename_torch = 'keane' + '_it_' + str(i) + '.torch'
