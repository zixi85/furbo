...
top = np.amax(ub) + 0.1 * np.abs(np.amax(ub))
middle = np.amax(mean)/2
bot = np.amin(lb) - 0.1 * np.abs(np.amin(lb))
...
if np.amax(ub) + 0.1 * np.abs(np.amax(ub)) > top:
    top = np.amax(ub) + 0.1 * np.abs(np.amax(ub))
if np.amax(mean)/2 > middle:
    middle = np.amax(mean)/2
if np.amin(lb) - 0.1 * np.abs(np.amin(lb)) < bot:
    bot = np.amin(lb) - 0.1 * np.abs(np.amin(lb))
    
middle = (top + bot)/2

# Add description
ax.set_title('Keane Bump (30D)')
...
# Save figure
fig.savefig(os.path.join(os.getcwd(), 'keane.png'), dpi=300)
