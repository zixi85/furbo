...
from objectives import welded_beam
from constraints import welded_beam_cons
import plotting
...
cwd_current = os.path.join(cwd_base, 'welded_beam')
    
# Check if directory already exists
if any([True if dir_ == 'welded_beam' else False for dir_ in os.listdir(cwd_base)]):
    # Check if problem was already solved
    if any([True if dir_ == 'complete' else False for dir_ in os.listdir(os.path.join(cwd_base, 'welded_beam'))]):
...
for i, seed in enumerate(seeds):
    ...
    if 'welded_beam' + '_it_' + str(i) + '.torch' in os.listdir(os.path.join(cwd_base, 'welded_beam')):
        continue
    ...
    filename_torch = 'welded_beam' + '_it_' + str(i) + '.torch'
