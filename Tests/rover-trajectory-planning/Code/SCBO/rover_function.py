...
def is_point_in_square(points, center, side_length):
    # x, y = points
    cx, cy = center
    half_side = side_length / 2
    
    return [(abs(p[0] - cx) <= half_side and abs(p[1] - cy) <= half_side) for p in points]
    
    # return abs(x - cx) <= half_side and abs(y - cy) <= half_side
...
class RoverTrajPlan():
    def __init__(self, dim=20):
        self.dim = dim
        if dim == 20:
            self.domain = create_small_domain()
        elif dim == 60:
            self.domain = create_large_domain(force_start=True,
                                              force_goal=True,
            ...
            print("Error!")
            exit()
        n_points = self.domain.traj.npoints
        raw_x_range = np.repeat(self.domain.s_range, n_points, axis=1)
        # maximum value of f
        f_max = 5.0
        ...
        self.info = "Rover Trajectory Planning"
        self.min = -5
    ...
    def cons(self):        
        ...
        spline_points = self.domain.traj.get_points(np.linspace(0, 1.0, 1000, endpoint=True))
        
        # Compute l2 norm on all spline points for all stones and save the smallest
        val = []
        for s in stones:
            
            distances = np.linalg.norm(spline_points - s, axis=1)
            
            mask = is_point_in_square(spline_points, s, stone_side)
            
            if any(mask):
                val.append(np.sum(distances[mask]))
            else:
                val.append(-1 * np.amin(distances))
    ...
    def plot(self, cwd):
        ...
        for t in trees:
            rect = Rectangle(t - 0.025, 0.05, 0.05, linewidth=1, edgecolor='none', facecolor='y')
            ax.add_patch(rect)
        
        for s in stones:
            rect = Rectangle(s - 0.05, 0.1, 0.1, linewidth=1, edgecolor='none', facecolor='r')
            ax.add_patch(rect)
        
        points = self.domain.traj.get_points(np.linspace(0, 1.0, 1000, endpoint=True))
        ax.plot(points[:,0], points[:,1], color = 'b')
        
        # Identify the last figure
        # Filter folders
        list_dir = [ name for name in os.listdir(cwd) if not os.path.isdir(os.path.join(cwd, name)) ]
        
        # If no figure, start from 0
        if not list_dir:
            i=0
            fig_name = os.path.join(cwd, str(i).zfill(3))
        # Else identify the last figure (only numbered figures)
        else:
            list_dir_int = [int(d[:-4]) for d in list_dir]
            i = np.amax(list_dir_int) + 1
            fig_name = os.path.join(cwd, str(i).zfill(3))
            
        plt.savefig(os.path.join(cwd, fig_name))
        plt.close()
        
        return

    def plot_best(self, cwd, x):
        ...
        self.eval(x)
        # ax.set_title(f"Cost: {self.eval(x):.4g}")
        
        points = self.domain.traj.get_points(np.linspace(0, 1.0, 1000, endpoint=True))
        
        ax.plot(points[:,0], points[:,1], color = 'b')
        ax.plot(points[0,0], points[0,1], color = 'w', marker='o')
        ax.plot(points[-1,0], points[-1,1], color = 'w', marker='x')
        ...
        plt.savefig(os.path.join(cwd, fig_name), dpi = 600)
        plt.close()
        

if __name__ == "__main__":
    rv = RoverTrajPlan(dim=60)
    tmp = rv(np.linspace(0,1,60))
    tmp_c = rv.cons()
    
    cwd = os.path.join(os.getcwd(), 'best_plots')
    
    rv.plot_best(cwd, np.linspace(0,1,60))