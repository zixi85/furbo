...
cwd_best = os.path.join(os.getcwd(), 'plots')
...
cwd_current = os.path.join(cwd_base, 'rover')
    
# Check if directory already exists
if any([True if dir_ == 'rover' else False for dir_ in os.listdir(cwd_base)]):
    # Check if problem was already solved
    if any([True if dir_ == 'complete' else False for dir_ in os.listdir(os.path.join(cwd_base, 'rover'))]):
...
for i, seed in enumerate(seeds):
    ...
    if 'rover' + '_it_' + str(i) + '.torch' in os.listdir(os.path.join(cwd_base, 'rover')):
        continue
    ...
    FuRBO_status = variant_one(obj = rover,                       # Objective function
                               cons = rover.cons,                     # Constraints function
    ...
    while not FuRBO_status.finish_trigger:
        ...
        while not FuRBO_status.restart_trigger and not FuRBO_status.finish_trigger:
            ...
            for x in X_next:
                ...
                Y_next.append(FuRBO_status.obj(x))
                # ... and constraints
                C_next.append(FuRBO_status.cons())
            ...
            if (FuRBO_status.best_C <= 0).all():
                ...
                FuRBO_status.obj.plot_best(cwd_best, FuRBO_status.best_X.cpu().numpy())
                # print(f"Y: {Y_next}")
            else:
    ...
    filename_torch = 'rover' + '_it_' + str(i) + '.torch'
