...
from objectives import spring
from constraints import spring_cons
import plotting
...
cwd_current = os.path.join(cwd_base, 'spring')
    
# Check if directory already exists
if any([True if dir_ == 'spring' else False for dir_ in os.listdir(cwd_base)]):
    # Check if problem was already solved
    if any([True if dir_ == 'complete' else False for dir_ in os.listdir(os.path.join(cwd_base, 'spring'))]):
...
for i, seed in enumerate(seeds):
    ...
    if 'spring' + '_it_' + str(i) + '.torch' in os.listdir(os.path.join(cwd_base, 'spring')):
        continue
    ...
    filename_torch = 'spring' + '_it_' + str(i) + '.torch'
