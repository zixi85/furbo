...
def plot_convergence(crv, ax):
    ...
    # Load data SCBO
    cwd = os.path.join(os.getcwd(), 'Experiments', 'p001')
    y_1, y_1_max = load_data_(crv, cwd)
    
    # Load data CEI
    cwd = os.path.join(os.getcwd(), 'Experiments', 'p005')
    y_5, y_5_max = load_data_(crv, cwd)
    
    # Load data COBYLA
    cwd = os.path.join(os.getcwd(), 'Experiments', 'p010')
    y_10, y_10_max = load_data_(crv, cwd)
    
    # Load data CMAES
    cwd = os.path.join(os.getcwd(), 'Experiments', 'p020')
    y_20, y_20_max = load_data_(crv, cwd)
        
    # Find worst feasible
    y_max = np.amax([y_1_max, y_5_max, y_10_max, y_20_max])
    ...
    top_5, middle_5 = plot_curve(ax, y_5, 'darkgreen')

    # Plot Random Sampling data
    y_10[y_10==np.amax(y_10)] = y_max
    top_10, middle_10 = plot_curve(ax, y_10, 'darkred')
    
    # Plot CMAES data
    y_20[y_20==np.amax(y_20)] = y_max
    top_20, middle_20 = plot_curve(ax, y_20, 'darkblue')
    
    top = np.amax([top_1, top_5, top_10, top_20])
    middle = np.amax([middle_1, middle_5, middle_10, middle_20])
...
# Add legend
patchList = [patches.Patch(color='darkorange', label='1 %'),
             patches.Patch(color='darkgreen', label='5 %'),
             patches.Patch(color='darkred', label='10 %'),
             patches.Patch(color='darkblue', label='20 %')]
...
# Save figure
fig.savefig(os.path.join(os.getcwd(), 'FuRBOperc' + '.png'), bbox_inches='tight', dpi=600)
