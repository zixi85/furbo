...
from objectives import pressure_vessel
from constraints import pressure_vessel_cons
import plotting
...
cwd_current = os.path.join(cwd_base, 'pressure_vessel')
    
# Check if directory already exists
if any([True if dir_ == 'pressure_vessel' else False for dir_ in os.listdir(cwd_base)]):
    # Check if problem was already solved
    if any([True if dir_ == 'complete' else False for dir_ in os.listdir(os.path.join(cwd_base, 'pressure_vessel'))]):
...
for i, seed in enumerate(seeds):
    ...
    if 'pressure_vessel' + '_it_' + str(i) + '.torch' in os.listdir(os.path.join(cwd_base, 'pressure_vessel')):
        continue
    ...
    FuRBO_status = variant_one(obj = obj,                         # Objective function
    ...
    while not FuRBO_status.finish_trigger:
        ...
        while not FuRBO_status.restart_trigger and not FuRBO_status.finish_trigger:
            ...
            for x in X_next:
                ...
                Y_next.append(-1 * FuRBO_status.obj(x))
                # ... and constraints
                C_next.append(FuRBO_status.cons(x))
    ...
    filename_torch = 'pressure_vessel' + '_it_' + str(i) + '.torch'
