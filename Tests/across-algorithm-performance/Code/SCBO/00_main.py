...
for p in suite:
    ...
    if not ('i01' in p.id or 'i02' in p.id or 'i03' in p.id):
        continue
    if not ('d02' in p.id or 'd10' in p.id or 'd40' in p.id or):
        continue
#    if not ('f005' in p.id):
        print('Skipped')
        continue
    ...
    # Optimization start
        ...
        SCBO_status = variant_one(obj = p,                          # Objective function
        ...
        while not SCBO_status.finish_trigger:
            ...
            while not SCBO_status.restart_trigger and not SCBO_status.finish_trigger:
                ...
                for x in X_next:
                    ...
                    Y_next.append(-1*SCBO_status.obj(unnormalize(x, [p.lower_bounds, p.upper_bounds])))
                    # ... and constraints
                    C_next.append(SCBO_status.cons(unnormalize(x, [p.lower_bounds, p.upper_bounds])))
