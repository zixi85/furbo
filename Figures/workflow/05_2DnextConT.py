...
grid_x = unnormalize(grid_x, [p.lower_bounds[0], p.upper_bounds[0]])

X_train = torch.concatenate([unnormalize(X_train, [p.lower_bounds[0], p.upper_bounds[0]]), Tensor(X_next).unsqueeze(-2)])
Y_train = torch.concatenate([Y_train, Tensor([p(X_next)]).unsqueeze(-1)])
C_train = torch.concatenate([C_train, Tensor([np.amax(p.constraint(X_next))]).unsqueeze(-1)])
Y_model = get_fitted_model(X_train, Y_train, p.dimension, max_cholesky_size = float("inf"))
C_model = get_fitted_model(X_train, C_train, p.dimension, max_cholesky_size = float("inf"))
...
# Add samples
ax.scatter(X_train.cpu().numpy()[:,0],
           X_train.cpu().numpy()[:,1],
           color = 'k')

# Add best sample
X_best = X_train[get_best_index_for_batch(1, Y_train, C_train)]
ax.scatter(X_best.cpu().numpy()[0][0],
           X_best.cpu().numpy()[0][1],
           color = 'r')
...
# Save figure
fig.savefig(os.path.join(cwd_save, '2DnextConT' + '.png'))
